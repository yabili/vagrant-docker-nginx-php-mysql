...
sudo apt-get install -y linux-image-extra-$(uname -r) linux-image-extra-virtual >> $VAGRANT_BUILD_LOG 2>&1
sudo apt-get install apt-transport-https ca-certificates >> $VAGRANT_BUILD_LOG 2>&1
sudo apt-key adv --keyserver hkp://ha.pool.sks-keyservers.net:80 \
                 --recv-keys 58118E89F3A912897C070ADBF76221572C52609D >> $VAGRANT_BUILD_LOG 2>&1
echo "deb https://apt.dockerproject.org/repo ubuntu-trusty main" | sudo tee /etc/apt/sources.list.d/docker.list >> $VAGRANT_BUILD_LOG 2>&1
sudo apt-get update >> $VAGRANT_BUILD_LOG 2>&1

sudo apt-get install -y docker-engine >> $VAGRANT_BUILD_LOG 2>&1
sudo groupadd docker >> $VAGRANT_BUILD_LOG 2>&1
sudo usermod -aG docker vagrant >> $VAGRANT_BUILD_LOG 2>&1
sudo docker --version >> $VAGRANT_BUILD_LOG 2>&1

echo "Restarting Docker service"
sudo service docker restart >> $VAGRANT_BUILD_LOG 2>&1

# Install Docker Compose
echo "Installing Docker Compose"
sudo curl -L "https://github.com/docker/compose/releases/download/1.9.0/docker-compose-$(uname -s)-$(uname -m)" -o /usr/local/bin/docker-compose >> $VAGRANT_BUILD_LOG 2>&1
sudo chmod +x /usr/local/bin/docker-compose >> $VAGRANT_BUILD_LOG 2>&1
docker-compose --version >> $VAGRANT_BUILD_LOG 2>&1
